#!/usr/bin/env bash
# ensure-iam_policy.sh — idempotent IAM policy for bucket access
#
# Creates a tenancy-level policy. If .iam_group.ocid is set (recommended), grants
# access to that group. Otherwise grants access to a single user via a dynamic
# where-clause on request.principal.id.
#
...
if [ -z "$POLICY_OCID" ] || [ "$POLICY_OCID" = "null" ]; then
  POLICY_OCID=$(oci iam policy create \
    --compartment-id "$TENANCY_OCID" \
    --name "$POLICY_NAME" \
    --description "oci_scaffold: IAM policy for $NAME_PREFIX" \
